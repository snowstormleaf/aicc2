  ...
  const parseCSV = (text: string): Feature[] => {
    ...
    const parsed = lines
      ...
      .map((line, index) => {
        ...
        if (Number.isNaN(materialCost)) throw new Error(`Row ${index + 2}: Invalid cost value "${costStr}"`);
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-2 text-center">
        <p className="type-caption">Step 3</p>
        <h2 className="type-headline">Upload Feature Data</h2>
        <p className="type-deck mx-auto content-measure">
          Upload CSV/XLSX and refine all feature rows in one editorial table before analysis.
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-start justify-between gap-3">
            ...
            <Button
              onClick={() => {
                const csvContent =
                  "Feature Name,Description,Material Cost (USD)\n" +
        ...
        <CardContent className="space-y-4">
          <div
            className={`rounded-lg border border-dashed p-6 text-center transition-colors duration-200 ${
              isDragOver ? "border-primary bg-primary/10" : "border-border-subtle hover:border-primary/40"
            }`}
            onDrop={handleDrop}
            onDragOver={(e) => {
              e.preventDefault();
          ...
          >
            ...
            <p className="mb-2 text-sm text-muted-foreground">Drag and drop your CSV/XLSX file here, or click to browse</p>
            <input
              type="file"
              accept=".csv,.xlsx,.xls"
          ...
          <Alert>
            ...
            <AlertDescription>
              Include columns <strong>Feature Name</strong>, <strong>Description</strong>, and <strong>Material Cost (USD)</strong>. Blank
              rows are ignored.
            </AlertDescription>
          </Alert>
        </CardContent>
          ...
          <Table>
            ...
            <TableBody>
              {editorRows.map((feature, index) => {
                const isEmptyRow =
                  index === editorRows.length - 1 && !feature.name && !feature.description && !feature.materialCost;
                return (
                  <TableRow key={`${feature.id || "new"}-${index}`}>
                    <TableCell className="py-2">
                      <input
                        className="w-full rounded-sm border border-transparent bg-transparent px-2 py-1 text-sm transition-colors duration-200 placeholder:text-muted-foreground focus-visible:border-border focus-visible:bg-background"
                        value={feature.name}
                    ...
                    <TableCell className="py-2 text-right">
                      <input
                        className="w-full rounded-sm border border-transparent bg-transparent px-2 py-1 text-right text-sm transition-colors duration-200 placeholder:text-muted-foreground focus-visible:border-border focus-visible:bg-background"
